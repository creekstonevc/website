...
export function createTtsTicket(
  ...
  if (!secret || secret.length < 32) {
    throw new Error("Gateway signing secret must contain at least 32 characters");
  }
...
export function verifyTtsTicket(ticket, secret, { now = Date.now() } = {}) {
  if (typeof ticket !== "string" || ticket.length < 32 || ticket.length > 64_000) {
    throw new GatewayError(400, "invalid_tts_ticket", "Voice ticket is invalid");
  }
  ...
  if (
    ...
    throw new GatewayError(403, "invalid_tts_ticket", "Voice ticket is invalid");
  }
  ...
  if (decoded.exp <= now) {
    throw new GatewayError(410, "expired_tts_ticket", "Voice ticket has expired");
  }
...
export function validateConversationId(value) {
  ...
  if (typeof value !== "string" || value.length > 256 || !/^[A-Za-z0-9._:-]+$/.test(value)) {
    throw new GatewayError(400, "invalid_conversation", "Conversation identifier is invalid");
  }
  return value;
}

export function buildBoidsResponsePayload(body, model, maxInputCharacters = 4_000) {
  ...
  const conversation = validateConversationId(body.conversation);
  return {
    model,
    input,
...
export function parseSseFrame(frame) {
  ...
  if (rawData === "[DONE]") return { type: eventName, payload: null, done: true };
...
export function extractCompletedText(payload, streamedText = "") {
  ...
  if (Array.isArray(response?.output)) {
    for (const item of response.output) {
      if (!item || typeof item !== "object" || (item.type && item.type !== "message")) continue;
...
export function decodeBytePlusAudio(rawText) {
  ...
  if (!audio.length) {
    throw new GatewayError(502, "tts_upstream_error", "Voice rendering returned no audio");
  }
  return audio;
}
...
export async function readJsonBody(request, maxBytes) {
  ...
  try {
    ...
    throw new GatewayError(400, "invalid_json", "Request body must contain valid JSON");
  }
}
